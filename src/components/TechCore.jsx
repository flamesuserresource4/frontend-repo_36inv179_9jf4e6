...
export default function TechCore() {
  return (
    <section id="tech" className="bg-amber-50 dark:bg-zinc-950">
      <div className="mx-auto max-w-6xl px-6 py-16 sm:py-20">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {items.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="group rounded-2xl border border-amber-200 dark:border-white/10 bg-white dark:bg-black p-6 shadow-sm transition hover:shadow-md">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">
                <Icon size={20} />
              </div>
